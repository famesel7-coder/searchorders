...
STATUSES=("new","interesting","review","rejected","contacted","replied","meeting","won","lost","ignored")
class ScanManager:
...
class SearchOrdersWebApp:
    ...
    def _form(environ:dict[str,Any])->dict[str,list[str]]:
        ...
        body=environ["wsgi.input"].read(min(length,4096)).decode("utf-8",errors="replace") if length else "";return parse_qs(body)
    def __call__(self,environ:dict[str,Any],start_response:Callable[...,Any])->list[bytes]:
        ...
        if path.startswith("/leads/") and path.endswith("/status") and method=="POST":
            parts=path.strip("/").split("/")
            try:lead_id=int(parts[1])
            except(ValueError,IndexError):return self._text(start_response,"Invalid lead id",status="400 Bad Request")
            status=(self._form(environ).get("status") or [""])[0]
            try:self.store.set_status(lead_id,status)
            except ValueError as exc:return self._text(start_response,str(exc),status="400 Bad Request")
            start_response("303 See Other",[("Location","/"),("Cache-Control","no-store")]);return[]
        ...
        if path=="/feedback.json" and method in {"GET","HEAD"}:return self._json(start_response,{"statuses":self.store.feedback_stats()},head=method=="HEAD")
    ...
    def _render(self)->str:
        ...
        for item in payload["items"]:
            lead=item.get("lead",{});score=item.get("score",{});cls=item.get("classification",{});budget=_money(lead);refs=int(item.get("source_count") or 0);catalog_id=int(item.get("catalog_id") or 0);status=str(item.get("catalog_status") or "new");source=html.escape(str(lead.get("source_name") or lead.get("source") or "unknown"));title=html.escape(str(lead.get("title") or "Без названия"));desc=html.escape(str(lead.get("description") or "")[:500]);url=html.escape(str(lead.get("url") or ""));bucket=html.escape(str(score.get("bucket") or "archive"));intent=html.escape(str(cls.get("intent") or "ambiguous"));deadline=html.escape(str(lead.get("deadline_text") or ""));budget_markup=f'<p class="budget">{html.escape(budget)}</p>' if budget else "";deadline_markup=f'<p class="deadline">Срок: {deadline}</p>' if deadline else "";link=f'<a href="{url}" target="_blank" rel="noreferrer">Открыть источник</a>' if url else "";options="".join(f'<option value="{s}"{" selected" if s==status else ""}>{s}</option>' for s in STATUSES);feedback=f'<form class="feedback" method="post" action="/leads/{catalog_id}/status"><select name="status">{options}</select><button type="submit">Сохранить</button></form>' if catalog_id else "";cards.append(f'<article><div class="top"><b>{bucket.upper()}</b><span>{score.get("total",0)}/100</span></div><h3>{title}</h3><p class="meta">{source} · {intent} · источников: {refs}</p>{budget_markup}{deadline_markup}<p>{desc}</p>{link}{feedback}</article>')
        sources=self.store.source_stats();source_rows="".join(f'<tr><td>{html.escape(str(x.get("source_name") or x.get("source_id")))}</td><td>{html.escape(str(x.get("source_type")))}</td><td>{int(x.get("posts_seen") or 0)}</td><td>{int(x.get("leads_found") or 0)}</td><td>{html.escape(str(x.get("last_error") or "—"))}</td></tr>' for x in sources)
        scan_label="Сканирование идёт…" if self.manager.running else "Запустить сканирование"
        return f'''<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{html.escape(self.title)}</title><style>body{{font-family:system-ui,sans-serif;margin:0;background:#f4f4f1;color:#171717}}main{{max-width:1180px;margin:auto;padding:24px}}header{{background:#111;color:white;padding:28px;border-radius:22px}}h1{{margin:0 0 8px}}button,select{{padding:10px 14px;border:1px solid #ccc;border-radius:999px;font-weight:700}}button{{cursor:pointer}}.stats{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:18px 0}}.stats div,article,.sources{{background:white;border:1px solid #ddd;border-radius:18px;padding:18px}}section{{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px}}.top{{display:flex;justify-content:space-between}}.meta{{color:#666;font-size:13px}}.budget,.deadline{{font-weight:800}}a{{color:#075ea8}}.feedback{{display:flex;gap:8px;margin-top:14px}}table{{width:100%;border-collapse:collapse}}td,th{{text-align:left;padding:8px;border-bottom:1px solid #eee;font-size:13px}}.sources{{margin-top:22px;overflow:auto}}@media(max-width:700px){{.stats{{grid-template-columns:repeat(2,1fr)}}}}</style></head><body><main><header><h1>{html.escape(self.title)}</h1><p>Постоянный каталог спроса из Telegram, VK и других community-источников. Биржи исключены.</p><button id="scan">{scan_label}</button></header><div class="stats"><div><b>{summary['total']}</b><br>Всего</div><div><b>{summary['hot']}</b><br>Hot</div><div><b>{summary['review']}</b><br>Review</div><div><b>{summary['archive']}</b><br>Archive</div></div><section>{''.join(cards) or '<article>Каталог пока пуст.</article>'}</section><div class="sources"><h2>Источники</h2><table><thead><tr><th>Источник</th><th>Тип</th><th>Постов</th><th>Лидов</th><th>Последняя ошибка</th></tr></thead><tbody>{source_rows or '<tr><td colspan="5">Нет данных</td></tr>'}</tbody></table></div></main><script>document.getElementById('scan').onclick=async()=>{{let r=await fetch('/scan',{{method:'POST'}});document.getElementById('scan').textContent=r.ok?'Сканирование запущено':'Уже выполняется';setTimeout(()=>location.reload(),2500)}};</script></body></html>'''
    @staticmethod
