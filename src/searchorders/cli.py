...
def build_parser()->argparse.ArgumentParser:
    ...
    fs=s.add_parser("feedback-status");fs.add_argument("--state",default="state/searchorders.db",type=Path);fs.add_argument("lead_id",type=int);fs.add_argument("status")
...
def run_sources_status(args)->int:
    ...
    return 0
def run_feedback_status(args)->int:CatalogStore(args.state).set_status(args.lead_id,args.status);print(f"Lead {args.lead_id} -> {args.status}");return 0
...
def main(argv:list[str]|None=None)->int:
    ...
    try:
        ...
        if args.command=="feedback-status":return run_feedback_status(args)
