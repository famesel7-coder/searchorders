...
def evaluate_lead(
    ...
    *,
    ...
    proposal = None
    if score.bucket in {"hot", "review"}:
        proposal = create_proposal_draft(
            lead,
            classification,
            matched_case,
            site_url=str(profile.get("proposal", {}).get("site_url", "https://imon.agency/")),
        )
...
def evaluate_leads(
    ...
    *,
    ...
    evaluations = [evaluate_lead(lead, profile, cases, now=now) for lead in leads]
    bucket_order = {"hot": 0, "review": 1, "archive": 2, "rejected": 3}
    return sorted(
        evaluations,
        key=lambda item: (bucket_order.get(item.score.bucket, 9), -item.score.total),
    )
